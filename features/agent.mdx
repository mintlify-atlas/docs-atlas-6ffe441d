---
title: "Autonomous Agent Mode with MCP Tool Support"
sidebarTitle: "Agent"
...
The following tools are available to the agent by default. Which tools appear in any given turn is determined by a RAG-based retrieval step that selects only the tools relevant to your request — keeping the context lean for smaller models.
...
<AccordionGroup>
  ...
  <Accordion title="read_file / write_file / edit_file — File access" icon="file">
    Read existing files, write new files, or make precise find-and-replace edits to existing files. Write and edit show a before/after diff in the tool bubble. File access is subject to the configured workspace, when one is set.
  </Accordion>
